...
int main(int argc, char ** argv)
{
  ...
  for (j=0; j<num_lines; j++) {
        ...
        if (current_scale == 1.0000) {
           if (j > 0) {
              tree_num++;
              if (tree_num == desired_halo) {
              FILE *out_file;
              char *filename = malloc(30*sizeof(char));
              snprintf(filename, sizeof(char) * 30, "tree_%d_%d_%d_halo_%d.txt",file_ident_1,file_ident_2,file_ident_3,tree_num); 
